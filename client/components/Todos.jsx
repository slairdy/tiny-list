    ...
    <>
      <div className="todos">
        {todoList.map((todo, i) => {
          return (
            <div key={i}>
              <Todo id={todo.id} index={i} task={todo.task} complete={todo.complete} />
            </div>
          )
        })}
        <input type="text" defaultValue="New Todo" id="newTodo" />
        <button onClick={handleCreateNew}>Add new Todo</button>
      </div>
    </>
  )
}
